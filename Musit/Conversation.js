...
messages[0].url = responseJson.tracks.items[0].external_urls.spotify  
  this.createNewConversation(this.state.selectedFriends);
...
selectFriendOrConversation(data) {
  ...
  this.setState((previousState) => {
  return {
    selectedFriends: previousState.selectedFriends.concat(data),
  };
});
}
  ...
  return (
    ...
    {!this.props.new ? (
      ...
      {this.state.enteringNames ? (
        ...
        renderRow={(data, sectionID, rowID) => <Contact {...data} row={rowID} navigator={this.props.navigator} onDonePress={(data) => this.selectFriendOrConversation(data)}/>}
        scrollEnabled={false}
        />
        ) : (!this.state.editing ? (
          <GiftedChat
